...
class AVHubertModel(BaseFairseqModel):
    ...
    def extract_finetune(self, source, padding_mask=None, mask=False, ret_conv=False, output_layer=None):
        ...
        #     x2 = x.clone().transpose(1, 2) # To make it compatible with positional encoding
        #     x2 = self.encoder.pos_conv(x2)
        #     x2 = x2.transpose(1, 2) # To make it compatible with transformer input
        #     layer_output, _ = layer_module(x2) 
        #     # The output of the current layer becomes the input for the next
        #     # Add the output of this layer to our list
        #     all_hidden_states.append(layer_output.clone())
        # # feature: (B, T, D), float
        # target: (B, T), long
        # x: (B, T, D), float
        # padding_mask: (B, T), bool
