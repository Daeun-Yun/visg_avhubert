...
class AVHubertSeq2Seq(FairseqEncoderDecoderModel):
    def __init__(self, encoder, decoder, tgt_dict, cfg):
        ...
        self.cfg = cfg
        self.freeze_finetune_updates = cfg.freeze_finetune_updates
        self.asr_weight = nn.Parameter(torch.tensor(1.0, dtype=torch.float32))  # weight for ASR loss
        self.viseme_weight = nn.Parameter(torch.tensor(0.0, dtype=torch.float32)) # weight for viseme classification loss
        self.log_sigma_asr = nn.Parameter(torch.tensor(0.0, dtype=torch.float32))  # log σ_asr
        self.log_sigma_viseme = nn.Parameter(torch.tensor(0.0, dtype=torch.float32))  # log σ_viseme
    ...
    def forward(self, **kwargs):
        ...
        if self.viseme_head is not None: # If viseme classification is enabled
            encoder_feats = output['encoder_out']  # T x B x C
            # Convert to B x T x C for classification
            encoder_feats = encoder_feats.transpose(0, 1)
            # Perform viseme classification
            viseme_logits = self.viseme_head(encoder_feats)  # B x T x 14
