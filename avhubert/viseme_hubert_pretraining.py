...
@dataclass
class Viseme_AVHubertPretrainingConfig(FairseqDataclass):
    data: str = field(
        default=MISSING, metadata={"help": "path to data directory"}
    )
    viseme_classification: bool = field(
        default=False,
        metadata={"help": "enable viseme classification"},
    )
    viseme_dir: Optional[str] = field(
        default=None,
        metadata={"help": "directory containing viseme transcription files (train/valid/test.vsm.txt)"},
    )
    use_ctc: bool = field(
        default=False,
        metadata={"help": "if true, use CTC loss for the viseme task"},
    )
...
@register_task("viseme_av_hubert_pretraining", dataclass=Viseme_AVHubertPretrainingConfig)
class Viseme_AVHubertPretrainingTask(FairseqTask):

    cfg: Viseme_AVHubertPretrainingConfig
    ...
    @classmethod
    def setup_task(
        cls, cfg: Viseme_AVHubertPretrainingConfig, **kwargs
    ...
    def load_dataset(self, split: str, **kwargs) -> None:
        ...
        viseme_path = None
        if self.cfg.viseme_classification and self.cfg.viseme_dir:
            viseme_path = os.path.join(self.cfg.viseme_dir, f"{split}.vsm")
            if not os.path.exists(viseme_path):
                logger.warning(f"Viseme file not found: {viseme_path}")
                viseme_path = None
            else:
                logger.info(f"Loading viseme transcriptions from: {viseme_path}")
        else:
            logger.info(f"Viseme classification is disabled, not loading viseme transcriptions")
