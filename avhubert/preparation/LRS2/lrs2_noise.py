# This code is based on the similar code from lrs3_noise.py
import os
import numpy as np
...
tsv_fn = os.path.join(lrs2, 'train.tsv')

output_wav = os.path.join(lrs2, 'noise', 'babble', 'noise.wav')
output_tsvs = [os.path.join(lrs2, 'noise', 'babble', 'valid.tsv'), os.path.join(lrs2, 'noise', 'babble', 'test.tsv')]
...
min_len = 15*sample_rate
speech_tsv_dir, speech_wav_dir = os.path.join(lrs2, 'noise', 'speech'), os.path.join(lrs2, 'noise', 'speech', 'wav')
...
for wav_fn in tqdm(wav_fns):
    ...
    wav_data = (wav_data / max(wav_data)) * 32767.0
