...
def main():
    ...
    # ↓↓↓↓↓ ADD THIS ↓↓↓↓↓
    parser.add_argument('--lrs2', type=str, help='LRS2 root (optional, will not overwrite MUSAN TSVs)')
    # ↑↑↑↑↑ ADD THIS ↑↑↑↑↑
    args = parser.parse_args()

    short_musan, output_tsv_dir = f"{args.musan}/short-musan", f"{args.musan}/tsv"

    # only run make_musan_tsv if MUSAN TSVs don't already exist
    if not os.path.exists(f"{output_tsv_dir}/all"):
        print(f"Make tsv for babble, music, noise")
        make_musan_tsv(short_musan, output_tsv_dir)
    else:
        print(f"Skipping MUSAN TSV creation (already exists at {output_tsv_dir})")

    # ↓↓↓↓↓ CHANGE START ↓↓↓↓↓
    if args.lrs2:
        print(f"Combine MUSAN + LRS2 speech noise")
        input_tsv_dirs = [f"{output_tsv_dir}/{x}" for x in ['noise', 'music', 'babble']] + [f"{args.lrs2}/noise/speech"]
        combine(input_tsv_dirs, f"{args.lrs2}/noise/tsv")  # write combined TSVs to LRS2, not MUSAN
    else:
        print(f"Combine MUSAN + LRS3 speech noise")
