## A copy of the avhubert/hubert_dataset.py file with some modifications
# Mainly for loading viseme information
...
class DictLabelEncoder:
    def __init__(self, dictionary):
        ...
        self.blank_idx = dictionary.get('<blank>', -1)

    def transform(self, labels):
        return [self.dictionary.get(l, self.dictionary.get('<unk>', -1)) for l in labels]
...
class AVHubertDataset(FairseqDataset):
    def __init__(
        ...
        self.viseme_labels = None
        self.use_ctc = use_ctc
        self.viseme_path = viseme_path
        ...
        # Check for viseme data paths
        if viseme_path is not None:
            if not os.path.exists(viseme_path):
                logger.warning(f"Viseme file not found: {viseme_path}")
            else:
                logger.info(f"Loading viseme labels from {viseme_path}")
                with open(viseme_path) as f:
                    vsm_labels = [line.strip().split() for line in f]
                    # Filter according to loaded indices
                    self.viseme_labels = [vsm_labels[i] for i in inds]
                    logger.info(f"Loaded {len(self.viseme_labels)} viseme sequences")
    ...
    def __getitem__(self, index):
        ...
        if self.viseme_labels is not None:
            if self.use_ctc:
                ...
                alignment_path = self.names[index][1].split(':')[0].replace('/audio/', '/audio/aligned/').replace('.wav', '_viseme_lee.csv')
                frame_times = np.loadtxt(alignment_path, delimiter=',', skiprows=1, usecols=(0, 1)) # Read timestamps
                frame_times = np.round(frame_times * 25).astype(int) # Convert to frames
                begin_times = frame_times[:, 0]
                ...
                for i, (start, end) in enumerate(zip(begin_times, end_times)): # Iterate through the begin and end times
                    ...
                    viseme_idx = min(i, viseme_len - 1)  # Prevent out-of-range errors
                    extended_labels[start:end] = viseme_list[viseme_idx]
                # Encode the viseme labels using the label encoder
                viseme_target = torch.LongTensor(
                self.label_encoder.transform(extended_labels)
            )
                item["viseme_target"] = viseme_target
                # Return viseme targets along with starting and ending times     
            return item
        else:
            return item
    ...
    def collater(self, samples):
        ...
        if "viseme_target" in samples[0]:
            if self.use_ctc is True:
                ...
                # The pad_idx should correspond to a value ignored by the loss.
                # Using the blank_idx (0) is safe here, as it's handled by CTC.
                viseme_pad_idx = self.blank_idx 

                collated_viseme_targets = data_utils.collate_tokens(
                    viseme_targets,
                    pad_idx=viseme_pad_idx,
                    left_pad=False
                )
                
                batch["viseme_target"] = collated_viseme_targets
                batch["viseme_target_lengths"] = viseme_lengths

                # The input lengths for CTC are the lengths of the encoder output sequences
                if net_input["padding_mask"] is not None:
                    non_padding = ~net_input["padding_mask"]
                    input_lengths = non_padding.long().sum(-1)
                    batch["viseme_input_lengths"] = input_lengths
            else:
                max_vsm_len = max(len(s["viseme_target"]) for s in samples) # find max length in terms of frames
